...
def find_gate(answer: torch.Tensor, threshold: float, start: bool):
    ...
    gate_candidates = (answer >= threshold).nonzero(as_tuple=True)  # Find all candidates globally

    if len(gate_candidates[0]) == 0:  # No gate found
        return None

    if start:
        return gate_candidates[0][0].item(), gate_candidates[1][0].item()  # First candidat found
    else:
        return gate_candidates[0][-1].item(), gate_candidates[1][-1].item()  # Last cadidat found
